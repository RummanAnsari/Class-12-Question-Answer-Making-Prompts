# WBCHSE Class XII COMA — English Explained Answer Paper Generator (Reusable Prompt)
...
>
> **How to use:** Copy the "MASTER PROMPT" box, fill the `>>> FILL THIS <<<` slots, paste to any AI, then compile the result with **pdfLaTeX** (Overleaf or local). This template is fully portable — it uses `fancyvrb` (not `listings`) and a text ER diagram (not `tikz`), so it compiles even on minimal TeX installs.
...
3. Write a DETAILED explanation:
   ...
   - GROUP C (Broad): thorough model answers. Where relevant include:
       * framed CODE blocks using the {code} environment (Python factorial via
         recursion; SQL create/insert/select)
       * comparison TABLES (List vs Tuple, OLTP vs OLAP)
...
## 🏆 GOLD TEMPLATE — English Explained Answer Paper (compiles with pdfLaTeX; fully portable)

> This is the exact, tested & compiled template. Keep this preamble and the `\Question{}{}` + `{answer}` pattern; only swap the question/answer content.
...
\chead{\small\textbf{COMA — Answer Key}}
...
\Question{1}{<<< MCQ question text >>> \quad (a) optA \ (b) optB \ (c) optC \ (d) optD}
\begin{answer}
...
\end{itemize}
\keyterm{(b) ...}
% code block example (uses fancyvrb {code}):
\begin{code}
def factorial(n):
    if n == 0 or n == 1:
        return 1
...
4. Done — no special fonts or packages needed. (Confirmed: 12 pages, compiles clean.)
...
- **Every question is repeated, then explained** — MCQs explain why the correct option wins AND why the others fail.
...
- **Group C** uses code blocks + comparison tables + a text ER diagram for full, exam-ready answers.
- **Filename convention:** `Paper<N>-A-English` (e.g. Paper1-A-English).
